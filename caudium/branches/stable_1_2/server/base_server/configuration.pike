...
//! defvar: _comment
//! An optional comment. This has no effect on the module, it is only a text field for comments that the administrator might have (why the module are here, etc.)
...
//!  name:  Module name
//
//! defvar: data_cache_size
//! The size of the data cache used to speed up requests for commonly requested files, in KBytes
//!  type: TYPE_INT
//!  name: Data Cache:Cache size
//
//! defvar: data_cache_file_max_size
//! The maximum size of a file that is to be considered for the cache
//!  type: TYPE_INT
//!  name: Data Cache:Max file size
//
//! defvar: data_cache_gc_cleanup
//! The amount of the cache space to clean up during a garbage collection run in percent. If you have a large cache, you can make this value lower. With a small cache and a small percentage the GC routine will run more often.
//!  type: TYPE_INT_LIST
//!  name: Data Cache:Garbage Collection Percentage
//
//! defvar: ErrorTheme
//! This is the theme to apply to any error messages generated 
//!  type: TYPE_STRING
...
//! This allows you to override the new style error responses and use 
//!  type: TYPE_FLAG
//!  name: Old-style 404's
//
//! defvar: ZNoSuchFile
//! <b>This is depreciated, and will only work when &quot;Old-style 404's&quot; is turned on.</b><br />What to return when there is no resource or file available at a certain location. $File will be replaced with the name of the resource requested, and $Me with the URL of this server 
//!  type: TYPE_TEXT_FIELD
//!  name: No such file Message (eg. 404 error)
//
//! defvar: comment
//! This text will be visible in the configuration interface, it  can be quite useful to use as a memory helper.
//!  type: TYPE_TEXT_FIELD|VAR_MORE
//!  name: Virtual server comment
//
//! defvar: name
//! This is the name that will be used in the configuration interface. If this is left empty, the actual name of the virtual server will be used
//!  type: TYPE_STRING|VAR_MORE
...
//!  name: Logging: Format
//
//! defvar: max_open_time
//! This variables sets the idle timeout before in seconds an opened log file is closed. The benefit of this variable is that little used virtual servers won't waste file descriptors. Set to zero to disable this feature.
//!  type: TYPE_INT_LIST
//!  name: Logging: Maximum idle time before closing
//
//! defvar: Log
//! Log requests
//!  type: TYPE_FLAG
...
//!  name: Logging: No Logging for
//
//! defvar: Domain
//! Your domainname, should be set automatically, if not, enter the correct domain name here, and send a bug report to <a href="mailto:caudium-bugs@caudium.net">caudium-bugs@caudium.net</a>
//!  type: TYPE_STRING
//!  name: Domain
//
//! defvar: Ports
...
//
//! defvar: FTPWelcome
//! FTP Welcome answer; transmitted to new FTP connections if the file <i>/welcome.msg</i> doesn't exist.
//!
//!  type: TYPE_TEXT_FIELD|VAR_MORE
//!  name: FTP:FTP Welcome message
//
//! defvar: named_ftp
//! Allow ftp to normal user-accounts (requires an authentication module, e.g. 'User database and security').
//!
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP:Allow named FTP
//
//! defvar: passive_ftp
//! Allow passive transfers on ftp server.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP:Allow passive FTP
//
//! defvar: restricpasv
//! Restrict passive FTP port to a specific range. For example when using Caudium as a FTP server behind a firewall or in a NATed environment.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP:Restrict Passive FTP ports
//
//! defvar: lowpasvport
//! The lowest port to use when Restrict Passive FTP ports is set.
//!  type: TYPE_INT|VAR_MORE
//!  name: FTP:Passive FTP lowest port
//
//! defvar: hipasvport
//! The highest port to use when Restrict Passive FTP port is set.
//!  type: TYPE_INT|VAR_MORE
//!  name: FTP:Passive FTP highest port
//
//! defvar: maxpasvtry
//! Number of trys to open a Passive port when Restrict Passive FTP is set. Caudium will random the use of the range. If it cannot open a port then passive transfert will failled.
//!  type: TYPE_INT|VAR_MORE
//!  name: FTP:Passive FTP max attempts
//
//! defvar: pasvnat
//! Enable NAT (Network Address Translation) support for Passive transferts. This allow you to specify the real IP address used by the NATed ftp server.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP:Passive FTP NAT support
//
//! defvar: anonymous_ftp
//! Allows anonymous ftp.
//!
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP:Allow anonymous FTP
//
//! defvar: guest_ftp
//! Allows FTP guest users.
//!
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP:Allow FTP guest users
//
//! defvar: ftp_user_session_limit
//! Limit of concurrent sessions a FTP user may have. 0 = unlimited.
//!
//!  type: TYPE_INT|VAR_MORE
//!  name: FTP:FTP user session limit
//
//! defvar: shells
//! File which contains a list of all valid shells
//!(usually /etc/shells). Used for named ftp.
...
//!  name: FTP:Shell database
//
//! defvar: ftpnohomedeny
//! Deny access to ftp named users when homedir exist.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: FTP: Deny named users with non-existant homedir
//

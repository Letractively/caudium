...
string tag_accessed(string tag, mapping m, object id)
{
  ...
  if(m->since) {
    ...
    {
     if(m->database)
      return rxmltags_module->tagtime(counter->creation_date(), m); // From rxmltags
     return rxmltags_module->tagtime(counter->creation_date(m->file), m);
    }

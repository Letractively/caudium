...
class http_error_handler {
     ...
     array err = catch {
	   ...
	   error_name = "No Error Message Supplied. Sorry.\n\n";
        ...
        else
        {
           ...
           {
              if (id->method != "GET" && id->method != "HEAD" && id->method != "POST")
                 error_message = "Method (" + _Roxen.html_encode_string (id->method) + ") not recognised.";
              else
                 error_message = "Unable to locate the file: " + _Roxen.html_encode_string (id->not_query) + ".<br />\n" +
  ...
  public mapping handle_error (int error_code, string error_name, string error_message, object id) {
    ...
    string error_page = Caudium.parse_html (get_template_data (local_template->name, id), ([ "error" : _tag_error ]), ([ ]), ([ "code" : error_code, "name" : error_name, "message" : error_message ]));

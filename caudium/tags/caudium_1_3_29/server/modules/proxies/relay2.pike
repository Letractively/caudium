...
class Relay
{
  ...
  void done_with_data( )
  {
	...
	return lambda( string t, mapping a ) {
		 ...
		 return ({Caudium.make_tag( p->tag_name(), a )});
	       };
      };
   ...
   // if( !options->cache ) NO_PROTO_CACHE();
    ...
    if( !headers || !data )
    {
      mapping q = Caudium.HTTP.string_answer( buffer, "" );
      q->raw = 1;
      id->send_result( q );
      destruct( );
      return;
	...
	do_rewrite( data );
      ...
      id->send_result( Caudium.HTTP.rxml_answer( query("pre-rxml")
                                               + data +
                                               query("post-rxml"), id ) );
    }
...
void create( object c )
{
  if( c )
  {
    defvar( "patterns", "", "Relay patterns", TYPE_TEXT,
	    ...
	    "arrived to Caudium, not even the Host: header is changed.  If "
	    ...
	    "<b>url-prefix</b> specified; no replacing is done.</p><p>"
            ...
            "If <b>LAST</b> is specified, the match is only tried if Caudium "

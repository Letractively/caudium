		     ...
		     mapping defines, object fd)
{
  ...
  if (args->query) {
    ...
    err=catch{ con=get_database_connection(args, request_id); };
	...
	contents = ("<h3>Couldn't connect to SQL-server</h1><br>\n" +
		    html_encode_string(err[0]) + "<false>");
      } else {
	contents = "<false>";
      }
    } else if (err = catch(result = con->query(args->query))) {
      if (!args->quiet) {
	if (args->log_error && QUERY(log_error)) {
	  report_error(sprintf("SQLTAG: Query %O failed:\n"
			       "%s\n",
			       args->query, describe_backtrace(err)));
	}
...
string sqlupdate_tag(string tag_name, mapping args,
                    object request_id, object f,
                    mapping defines, object fd)
{
  object(Sql.sql) con;
  ...
  if(!args->unique && !args->tablename && !args->scope)
    report_error("SQLTAG: missing arguments, need: unique, tablename, scope");
...
Sql.sql get_database_connection(mapping args, object request_id)
{
    ...
    if (sql_connect) {
      con = sql_connect(host);
    } else {
      host = (lower_case(host) == "localhost")?"":host;
      con = Sql.sql(host, database, user, password);
    }
    return con;
}
		    ...
		    mapping defines, object fd)
{
  ...
  if (args->query) {
    ...
    } else if (err = catch(res = con->query(args->query))) {
      if (!args->quiet) {
	if (args->log_error && QUERY(log_error)) {
	  report_error(sprintf("SQLTAG: Query %O failed:\n"
			       "%s\n",
    ...
    if(args->save)
    {
      if(!request_id->misc) request_id->misc=([]);
      if(!request_id->misc->sqlquery) request_id->misc->sqlquery=([]);
      request_id->misc->sqlquery[args->save]=res;
    }
    return("<true>");
  } else {
		    ...
		    mapping defines, object fd)
{
  ...
  if (args->query) {
    ...
    } else if (err = catch(result = con->big_query(args->query))) {
      if (!args->quiet) {
	if (args->log_error && QUERY(log_error)) {
	  report_error(sprintf("SQLTAG: Query %O failed:\n"
			       "%s\n",
    ...
    if (result) 
      res=format_result(result, args, request_id);
    else {
      if (ascii) {
	return("<false>");
      }
  ...
  } else if(args->savedquery && request_id->misc->sqlquery && request_id->misc->sqlquery[args->savedquery])
      res=format_result(request_id->misc->sqlquery[args->savedquery], args, request_id);
  else {
    if (ascii) {
      return("<false>");
    }
...
string format_result(object(Sql.sql_result) result, mapping args, object request_id)
{
      int ascii;
      string res;
      if (args->ascii) {
...
{
  return( ([ "sql":sqlquery_tag, 
             "sqlquery":sqlquery_tag,
	     "sqltable":sqltable_tag,
	     "sqlupdate":sqlupdate_tag,
#if 0
	     "dumpid":dumpid_tag
#endif /* 0 */
  ]) );
...
object(Sql.sql) sql_object(void|string host)
{
  ...
  mixed err;
  err = catch(con = sql_connect(host));
  if(err)
    return 0;
  return con;
}
...
void create()
{
  m_delete(master()->programs,search(master()->programs,(program)"sqlupdate.pike"));

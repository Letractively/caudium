...
{
  if(!host)     host=oh;
  lfile->write("[" + Caudium.HTTP.cern_date(time(1)) + "] ftp://" +
		 host + ":" + id + "\t" + more + "\n");
}
...
{         
  ...
  defvar("Proxies", "", "Remote gateway regular expressions",
	 ...
	 "Currently, <b>remote gateway has to be a http-ftp gateway</b> like this one.</p>"
	 "<p>"
...
//! defvar: mountpoint
//! By default, this is ftp:/. If you set anything else, all normal WWW-clients will fail. But, other might be useful, like /ftp/. if you set this location, a link formed like  this: &lt;a href="/ftp/"&lt;my.www.server&gt;/a&gt; will enable accesses to local WWW-servers through a firewall.<p>Please consider security, though.</p>
//!  type: TYPE_LOCATION|VAR_MORE
//!  name: Location
//
//! defvar: Proxies
//! Here you can add redirects to remote gateways. If a file is requested from a host matching a pattern, the gateway will query the Ftp gateway server at the host and port specified.<p> Hopefully, that gateway will then connect to the remote ftp server.<br />Currently, <b>remote gateway has to be a http-ftp gateway</b> like this one.</p><p>Example:<hr noshade="yes" /><pre># All hosts inside *.rydnet.lysator.liu.se has to be<br /># accessed through lysator.liu.se<br />.*\.rydnet\.lysator\.liu\.se        130.236.253.11  21
//!</pre></p>Please note that this <b>must</b> be regular expressions.
//!  type: TYPE_TEXT_FIELD|VAR_MORE
//!  name: Remote gateway regular expressions
//
//! defvar: method
//! What method to use to transfer files. 
//!  type: TYPE_STRING_LIST|VAR_MORE
//!  name: FTP transfer method
//
//! defvar: keeptime
//! How long time in <b>seconds</b> a connection to a ftp server is kept without usage before 
...
//! Hold data transfer until response from server; 
//!  type: TYPE_STRING_LIST|VAR_MORE
//!  name: Hold until response
//
//! defvar: connection_timeout
//! Time in seconds before a <i>connection attempt</i> is retried (!).
//!  type: TYPE_INT|VAR_MORE
//!  name: Connection timeout
//
//! defvar: data_connection_timeout
//! Time in seconds before a <i>data connection</i> is timeouted and cancelled.
//!  type: TYPE_INT|VAR_MORE
//!  name: Data connection timeout
//
//! defvar: save_dataports
//! Some ftpd's have problems when the same port is reused. Try this out on your own. :)
//!  type: TYPE_STRING_LIST|VAR_MORE
//!  name: Save dataports
//

			       ...
			       mapping hl)
{
    ...
    switch (tag) {
	case "channel":
	    Caudium.parse_html(contents, ([]), ([ "link" : parse_channel, "description" : parse_channel,
			       ...
			       mapping hl, mapping|void chhl)
{
  ...
  case "item":
    hl = ([]);
    Caudium.parse_html(contents, ([ ]), ([ "title": parse_it, "link": parse_it, "description" : parse_it  ]),
	       hl, chhl);
    if (chhl)
	hl += chhl;
    headlines += ({ hl });
...
private static void rdf_parse_reply(string data)
{
  ...
  Caudium.parse_html(data + " ", ([]), (["channel" : parse_channel ]), chhl );
  Caudium.parse_html(data + " ", ([]), (["item" : parse_it ]), 0, chhl );
}

...
public mixed try_get_file(string s, object id, int|void status, int|void nocache)
{
  ...
  if(!id->pragma["no-cache"] && !nocache && !id->auth)
    if(res = cache_lookup("file:"+id->site_id, s))
      return res;
  ...
  cache_set("file:"+id->site_id, s, res);
  return res;
}
...
// for now only theese two. In the future there might be more variables.
  ...
  defvar("PostBodySize", POST_MAX_BODY_SIZE, 
	 ...
	 TYPE_INT, "This variable determines how large (in bytes) a body of a POST "
	 "request could become before rejecting the request. If the value is -1 the size is "
         "unlimited. A value of 0 ignores the body completely.");

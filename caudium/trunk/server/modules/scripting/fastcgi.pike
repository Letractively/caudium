...
class FCGI
{
  static
  {
    ...
    void do_connect( object fd, mixed|void q )
    {
      ...
      werror(" Connecting...\n" );
#endif
      //fd->connect( "127.0.0.1",(int)(socket->query_address()/" ")[1]);
      fd->connect( "127.0.0.1",(int)Caudium.get_port(socket->query_address()));
    }
    ...
    void destroy()
    {
      report_warning("killing FastCGI processes.\n");
      foreach( all_pids, int p )
      {
        kill(p, signum("SIGTERM"));
      }
    }
    ...
    {
      socket = Stdio.Port(0, 0, "127.0.0.1" );

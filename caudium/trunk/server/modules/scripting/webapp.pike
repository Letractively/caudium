...
#define parse_error(X) return Caudium.HTTP.string_answer(X, "text/html") + \
  (["_parseerror": 1]) 
...
// hold the pike wrapper for CaudiumServletContext
object conf_ctx;
...
void do_parse_servlet(Node c, mapping(string:mixed) data)
{
      ...
      if (servlets[data["servlet-name"]])
        {
          report_error("Duplicate entry of %s in web.xml\n",
                       data["servlet-name"]);
          status_info+=sprintf("<pre>Duplicate entry of %s in web.xml</pre>",
                              data["servlet-name"]);
        }
      else
        {
...
void start(int x, object conf)
{
  ...
  if(query("searchpath"))
    normalized_path=QUERY(searchpath);
  if(query("mountpoint"))
  ...
  if(warname=="servlets/NONE") {
    status_info = (string) "No Web Application selected";
    return;
  }
  ...
  mixed exc = catch
  {
    ...
    WEBAPP_WERR("loading webfile: " + webfile);
  ...
  mixed exc2 = catch {
    ...
    conf_ctx = Servlet.context(conf, this_object(), "../var/servlettmp/");
  };
  
  if(exc2)
  {
    report_error("Servlet: %s\n",describe_backtrace(exc2));
    status_info+=sprintf("<pre>%s</pre>",describe_error(exc2));
  }
  else
  {
          ...
          lambda(string serv) {
            ...
            if (servlets[serv]->initialized == 1)
            ...
            else if (servlets[serv]->loaded == -1)
              {
                ...
                ret += "<b>Servlet failed to load!</b>";
                ret += "</font>";
              }
              ...
              {
                ret += "<font color='&usr.warncolor;'>";
                ret += "<b>Servlet failed to initialize!</b>";
                ret += "</font>";
              }
            ret += "</td>";
...
string query_name()
{
  ...
  if (sizeof(name) > 20) {
    return sprintf("Java: WAR loaded from %s...%s",
		   name[..7], name[sizeof(name)-8..]);
  }
  return sprintf("Java: WAR loaded from %s", name);
}
        ...
        foreach(headers[1..], string h) {
          ...
          if (sscanf(h, "%s:%s", name, value) == 2) {
            ...
            if ( !ignore_heads[lower_case(name)] )
              Caudium.add_http_header(headermap, name,
...
class RXMLParseWrapper
{
  ...
  mapping get_result()
  {
    ...
    Caudium.add_http_header(headermap, "Cache-control",
                          "no-cache");
    ...
    if (collect == 1)
    {
      res = Caudium.HTTP.rxml_answer(get_data(1), _id);
...
int load_servlet(mapping(string:string|int|mapping|Servlet.servlet) servlet)
{
#if constant(thread_create)
  ...
  if (mappingp(servlet) && !servlet->loaded)
    {
      if (classname) {
      ...
      if (e)
        {
          if (is_unavailable_exception(e))
          ...
          else
            {
              ...
              status_info=sprintf("<pre>%s</pre>",e[0]);
            }
        }
    }
}
...
{
  mapping(string:string|array|int|mapping|Servlet.servlet) ret;
...
mixed find_file( string f, RequestID id )
{
  ...
  WEBAPP_WERR("id->misc: " + sprintf("%O", id->misc));  

  servlet = map_servlet(f, id);

  WEBAPP_WERR("servlet: " + sprintf("%O", servlet));  

  WEBAPP_WERR("servlets: " + sprintf("%O", servlets));
  WEBAPP_WERR("servletmaps: " + sprintf("%O", servletmaps));

  if (!servlet) {
    WEBAPP_WERR("no servlet found.");
    if (!is_special(f, id)) {
      id->misc->path_info = path_info;
    WEBAPP_WERR("passing through to the filesystem.");
      return ::find_file("/" + f, id);
    }
    else 
    {
      WEBAPP_WERR("falling through (no file found).");
      return 0;
    }
  }
  else {
    if (servlet->initialized == 1)
      {
	    ...
	    id = id->clone_me();
            ...
            if (x == 0xffff || (x>=3 && chain_wrapper->collect)) {
              id->misc->cacheable = 5;
              return Caudium.HTTP.low_answer(500, "<title>Servlet Error - chaining failed</title>"
      ...
      if (servlet->permanent)
        {
          ...
          return Caudium.HTTP.low_answer(503, "<h1>Error: 503</h1>"
                                 "<h2>Location: " +
                                 loc + f + "</h2>"
...
mixed call_servlet(RequestID id, string f, string name )
{
  string oldf = f;
  WEBAPP_WERR("Request for \""+f+"\"" +
		  (id->misc->internal_get ? " (internal)" : ""));
    ...
    if (servlet->initialized == 1)
      {
	    ...
	    id->request_headers = request_headers;
            ...
            if (x == 0xffff || (x>=3 && chain_wrapper->collect)) {
              id->misc->cacheable = 5;
              parse_error( "Servlet Error - chaining failed\n"
                                  "Location: " +
                                  loc + f + "\n"
        ...
        parse_error("Internal Error! Collect not set!\n");
      }
    else if  (servlet->initialized == -1)
      if (servlet->permanent)
        {
          WEBAPP_WERR("Permanently unavailable detected in call_servlet");
          parse_error("Permanently Unavailable\n"
                             "Location: " +
                             loc + f + "\n"
      ...
      else
        {
          ...
          id->misc->cacheable = servlet->retry;
          parse_error("Service Unavailable\n"
            "Location: " +
            loc + f + "\n" +
            servlet->exc_msg + "\n"
            "Service is unavailable, try again in " +
...
mixed tag_servlet(string tag_name, mapping args, RequestID id, object 
     file, mapping defines) {
  WEBAPP_WERR(sprintf("args=%O\n%s\n", args, query_name()));
  if(!args->uri && !args->name)
  parse_error("Neither uri nor name specified.\n");
  foreach (id->conf->get_providers("webapp"), object m)
  {
    WEBAPP_WERR(sprintf("module:%O\n", m->query_name()));
    if (m->query("tagtarget") == args->webapp)
    {
      if(!objectp(id))
      error("No ID passed to tag 'servlet'\n");
      ...
      if (args->uri) {
       ...
       uri = Caudium.fix_relative (uri, id);
       ...
       mapping hdrs = m->call_servlet(fake_id, f, args->name || "");

       if(hdrs["_parseerror"]) return hdrs;

       CACHE( fake_id->misc->cacheable );
       destruct (fake_id);
          
       if (!mappingp(hdrs) && !objectp(hdrs)) {
         report_error("tag_servlet(%O): uri = %O\n   hdrs = %O is not a mapping.\n",
                     id, uri, hdrs);
       }
       mapping return_map=(["error":200, "type": "text/html", "data":
	sprintf("tag_servlet(%O): uri = %O\n   hdrs = %O is not a mapping.\n",
                     id, uri, hdrs)]);

       if (!args["no-headers"]) {
         if (hdrs->error && hdrs->error != 200) {
           return_map->error=hdrs->error;
	   if (hdrs->rettext)
             return_map->data=hdrs->rettext;
	   }
	   if (hdrs->extra_heads)
	     return_map->extra_heads=hdrs->extra_heads;
	 }
         if(!return_map->data) return_map->data=hdrs["data"];
 
         return_map->len=strlen(return_map->data);
...
{
  return 
#"<font color='&usr.warncolor;'>Java 2 is not available in this Caudium.<p>
  To get Java 2:
  <ol>
...
// we don't currently use this class
/*
class ClassPathList
{
...
void create()
{
  ...
  ::create();
// this is supposed to make a bunch of inherited defvars invisible.
// don't think caudium supports this...

  set_hidden("searchpath", 1);
  set_hidden(".files", 1);
  set_hidden("dir", 1);
  set_hidden("tilde", 1);
  set_hidden("put", 1);
  set_hidden("delete", 1);
  set_hidden("check_auth", 1);
  set_hidden("access_as_user", 1);
  // end filesystem
  ...
  defvar("rxmltypes", "text/xml text/html", "RXMLTypes", TYPE_TEXT,
	 "Content types that should be passed on to the "
                "RXML Parser.", 0,
         lambda() { return !query("rxml"); } );

  defvar("codebase", ({""}), "Class path", TYPE_FILE_LIST,
         "Any number of directories and/or JAR "
         "files from which to load the support classes." );
  ...
  defvar("jspengine", "None",
         "Servlet engine", TYPE_MULTIPLE_STRING,
...
//! defvar: warname
//! The archive file (.war) or directory containing the Web Application.
//!  type: TYPE_FILE
//!  name: Web Application Archive
//
//! defvar: rxml
//! If this is set, the output from the servlets handled by this module will be RXML parsed. NOTE: No data will be returned to the client until the output is fully parsed.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: Parse RXML in servlet output
//
//! defvar: rxmltypes
//! Content types that should be passed on to the RXML Parser.
//!  type: TYPE_TEXT
//!  name: RXMLTypes
//
//! defvar: codebase
//! Any number of directories and/or JAR files from which to load the support classes.
//!  type: TYPE_FILE_LIST
//!  name: Class path
//
//! defvar: parameters
//! Parameters for all servlets on the form <tt><i>name</i>=<i>value</i></tt>, one per line.
//!  type: TYPE_TEXT
//!  name: Parameters
//
//! defvar: jspengine
//! Select the jsp engine that should handle files with the .jsp extension.
//!  type: TYPE_MULTIPLE_STRING
//!  name: Servlet engine
//
//! defvar: jspdebug
//! Enable debug output from the JSP engine.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: JSP Debug
//
//! defvar: jspsenderrtoclient
//! Return jsp compilation errors to the client.
//!  type: TYPE_FLAG|VAR_MORE
//!  name: Send Errors To Client
//
//! defvar: tagtarget
//! Target name to use in the servlet tag to reference this Web Application. Leave empty to exclude from the servlet tag.
//!  type: TYPE_STRING|VAR_MORE
//!  name: Target name
//
//! defvar: anyservlet
//! Use a servlet mapping that mounts any servlet onto &lt;Mount Point&gt;/servlet/
//!  type: TYPE_FLAG|VAR_MORE
//!  name: Access any servlet
//
//! defvar: preloadall
//! Load all servlets at module initialization time even if load-on-startup is not specified in web.xml
//!  type: TYPE_FLAG|VAR_MORE
//!  name: Preload all servlets
//

...
#undef LIMIT
    ...
    if (id->misc->vhs && id->misc->vhs->host)
    {
       global_env["SERVER_URL"] = "http://" + id->misc->vhs->host + "/";
       global_env["SERVER_NAME"] = id->misc->vhs->host;
    }
    else
    {
    ...
    if (id->misc->vhs && id->misc->vhs->homedir)
    {
       global_env["HOMEDIR"] = id->misc->vhs->homedir;
...
string real_file( string f, object id )
{
  ...
  if (!id->misc->vhs) return 0;

  return combine_path( id->misc->vhs->cgipath, f );
}
...
array(string) find_dir(string f, object id)
{
  ...
  if (QUERY(ls)) return get_dir(real_file(f, id));
}
...
void create(object conf)
{
  ...
  defvar("cgilog", GLOBVAR(logdirprefix)+
	 Caudium.short_name(conf? conf->name:".")+"/cgi.log", 
  ...
  defvar("rawauth", 1, "Raw user info", TYPE_FLAG|VAR_MORE,
  ...
  defvar("clearpass", 1, "Send decoded password", TYPE_FLAG|VAR_MORE,

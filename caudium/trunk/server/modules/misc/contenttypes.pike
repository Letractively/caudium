...
//! defvar: exts
//! This is file extension to content type mapping. The format is as follows:<br /><br />
//!<i>&lt;extension&gt;&lt;one or more tabs&gt;&lt;content-type&gt;</i><br /><br />
//!<pre>	aif		audio/x-aiff</pre>
//!<pre>	ppm		image/x-portable-pixmap</pre>
//!<pre>	xls		application/vnd.ms-excel</pre>
//!<br />You can use the <b>#include &lt;<i>filename</i>&gt;</b> directive to include files. The <i>filename</i> argument may be an absolute or a relative path (in the latter case, it has to be relative to the server's root directory. For a list of types, see <a href="ftp://ftp.isi.edu/in-notes/iana/assignments/media-types/media-types">ftp://ftp.isi.edu/in-notes/iana/assignments/media-types/media-types</a>
//!  type: TYPE_TEXT_FIELD
//!  name: Extensions
//
//! defvar: extsfile
//! This is basically the same as the above, but intended for external (to Caudium) MIME-type databases, like /etc/mime.types on Debian/GNU Linux systems.<br />The format of this file can be the same as for Extensions, or like this:<br /><br />
//!<i>&lt;content-type&gt;&lt;one or more tabs&gt;&lt;extension&gt;</i><br /><br />
//!<pre>	audio/x-aiff	aiff</pre>
//!<pre>	image/x-portable-pixmap		ppm</pre>
//!<pre>	application/vnd.ms-excel	xls</pre>
...
//!
//!  type: TYPE_STRING
//!  name: System-wide extension file
//
//! defvar: default_ct
//! This is the default content type which is used if a file lacks extension or if the extension is unknown to the module.<br /><font color="#FF0000"><b>It is strongly recommended that you do leave this "application/octet-stream" at all times.<b></font><br />
//!
//!  type: TYPE_STRING
//!  name: Default content type
//
//! defvar: encs
//! This is file extensions to content encodings mapping. These rules tell Caudium how to set the Content-encoding header for different types of files.
//!
//!  type: TYPE_TEXT_FIELD
//!  name: Encodings
//

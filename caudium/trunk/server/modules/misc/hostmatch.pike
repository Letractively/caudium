...
void create() {
  defvar("matches", "",  "Regular expression rewrite rules",
	 ...
	 "<pre>"
	 "	^www\\.[^\\.]*\\.domain\\.com	users.domain.com\n"
	 "	sales\\.theweb\\.com		www.theweb.com\n"
...
//! defvar: Regular expression rewrite rules
//! Rules used to rewrite the URL to match an existing virtual server. Use this if you have hosts with multiple names or for example want to do wildcard DNS urls, like www.user.host.com. Examples: <p><pre>	^www\.[^\.]*\.domain\.com	users.domain.com
//!	sales\.theweb\.com		www.theweb.com
//!	.*\.whatever\.com		www.whatever.com
//!	.*				www.default.com
//!</pre>The latter will match every host not previously matched and can be used for setting a default site. <b>This includes all requests that lack a host header!</b> If a request is done to an ip-address and the Host header is sent, matching will be done as usual. Also note that all matching is done on the lower case host header without the :port part.

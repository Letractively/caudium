  ...
  object f)
{
  ...
  array s=id->conf->get_providers("emit." + lower_case(args->source));

  if(!s || !sizeof(s)) return "";

  array dataset = s[0]->get_dataset(args, id);

  string retval="";

  foreach(dataset, mapping row)
  {
    object newid = id->clone_me();

...
#if constant(Gdbm) && constant(Gdbm.gdbm)
...
#else // Gdbm && Gdbm.gdbm
constant module_doc  = "CIF authentication module that stores data in a GDBM file. Support for Gdbm is not available in your Pike, please recompile Pike "
"with Gdbm support.";
#endif
...
//! defvar: order
//! All authentication plugins have their call order assigned. It can be used to create chains of authentication. Plugins with the same order will be queried in an unpredicted order.
//!  type: TYPE_INT
//!  name: Authentication order
//
//! defvar: dbfile
//! Path to a file which will be used to store the authentication data. If the file doesn't exist, it will be created. 
//!  type: TYPE_FILE
//!  name: Database file
//
//! defvar: dbfileowner
//! uid:gid pair specifying the database file owner. If empty it will default to the user/group under which Caudium is running. You are encouraged to set it to some other user/group (but remember that in this case your Caudium must not drop the superuser privileges completely).
//!  type: TYPE_STRING
//!  name: Database file owner
//

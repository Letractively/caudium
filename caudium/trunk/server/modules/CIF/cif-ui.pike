...
//! defvar: themesdir
//! A directory with one subdirectory per theme. At least one subdirectory called <tt>default</tt> must exist in the indicated directory. Each theme directory contains one file per screen (the file can use other files in the directory, of course) and it can contain a directory called <tt>images</tt> which will be available on the runtime under the themename/images/ directory (relative to the mountpoint).
//!  type: TYPE_DIR

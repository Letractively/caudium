...
//! defvar: location
//! The mountpoint in the virtual filesystem for this module
//!  type: TYPE_LOCATION
//!  name: Mountpoint
//
//! defvar: search_mp3
//! The path in the real filesystem to the MP3 directory
//!  type: TYPE_STRING
//!  name: Search Paths: MP3 Files
//
//! defvar: search_promo
//! The path int the real filesystem to promo's if they are enabled
//!  type: TYPE_STRING
//!  name: Search Paths: Promo's
//
//! defvar: listing_streams
//! Enable listing of available streams
//!  type: TYPE_FLAG
//!  name: Listing: Stream Directory
//
//! defvar: listing_incoming
//! Enable listing of available incoming streams
//!  type: TYPE_FLAG
//!  name: Listing: Incoming Directory
//
//! defvar: listing_playlists
//! Enable listing of available playlists
//!  type: TYPE_FLAG
//!  name: Listing: Playlist Directory
//
//! defvar: maxclients
//! Maximum connected clients (per stream). Zero is infinite (*very dangerous*)
//!  type: TYPE_INT
//!  name: Clients: Maximum Clients
//
//! defvar: sessiontimeout
//! Target client session length (in seconds). This option disconnects a client from a stream at the end of a track, once they have been on longer than this. If you have very long tracks it's not worth setting this at all.
//!  type: TYPE_INT
//!  name: Clients: Maximum Session Length
//
//! defvar: pauseplayback
//! Pause the &quot;playback&quot; of streams when there are no clients listening to them
//!  type: TYPE_FLAG
//!  name: Clients: Pause Playback
//
//! defvar: titlestreaming
//! Enable streaming of track titles to clients, this is known to cause issues with some MP3 players
//!  type: TYPE_FLAG
//!  name: Clients: Title Streaming
//
//! defvar: allowtrackskip
//! Enable track skip from clients
//!  type: TYPE_FLAG
//!  name: Clients: Enable Track Skipping
//
//! defvar: allowtrackrepeat
//! Enable track repeat from clients
//!  type: TYPE_FLAG
//!  name: Clients: Enable Track Repeat
//
//! defvar: promos_enable
//! Enable Promo Streaming
//!  type: TYPE_FLAG
//!  name: Promo's: Enable Promo's
//
//! defvar: promos_freq
//! Insert Promo into stream every how many tracks?
//!  type: TYPE_INT
//!  name: Promo's: Frequency
//
//! defvar: promos_shuffle
//! If selected then the Promo's will be randomly ordered, otherwise they will be ordered alphabetically
//!  type: TYPE_FLAG
//!  name: Promo's: Shuffle
//
//! defvar: incoming_enable
//! Allow incoming stream connections?
//!  type: TYPE_FLAG
//!  name: Incoming Streams: Enable Incoming Streams
//
//! defvar: incoming_password
//! Password for incoming streams
//!  type: TYPE_STRING
//!  name: Incoming Streams: Password
//

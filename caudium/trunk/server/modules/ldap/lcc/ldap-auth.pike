...
//
//! defvar: user_dntype
//! This module can authenticate users based on number of LDAP attributes. It is possible to create the DN using the <code>uid</code> attribute, the <code>mail</code> attribute or both of them simultanously. <br />Below is a short explanation of what choices the adminstrator has:<br /><blockquote><ul><li><strong><code>any</code></strong>. This means that the module will attempt to authenticate the user using any of the two attributes mentioned above. If the login typed by the user contains the <code>@</code> character the login string will be treated as an email address and the user will be sought for using the <code>mail</code> attribute. If there's no <code>@</code> character in the login string, the search will be performed on the <code>uid</code> attribute. If the <em>Fallback to UID</em> option below is set then, should the <code>mail</code> search fail, the module will attempt the <code>uid</code> authentication.</li><li><strong><code>uid</code></strong>. The search is performed only on the <code>uid</code> attribute no matter what type of string the user entered in the login box. If the string is an email address, though, only the part before the <code>@</code> character will be used.</li><li><strong><code>email</code></strong>. The search is performed on the <code>mail</code> attribute. If the string entered by the user in the login box comes without the <code>@</code> character the further action depends upon the value of the <em>Use default domain</em> option. If the option is unset then the authentification will fail and if it exists the domain name (or names - see the <em>Use default domain</em> description) will be suffixed to the typed string and the authentication will be retried.</li><li><strong><code>both</strong></code>. This setting means that the user search will be performed using both the <code>uid</code> and the <code>mail</code> attributes. In this case the rules described in the previous section apply.</li><li><strong><code>other</strong></code>. This allows the administrator to specify the filter to be used to search the directory. The <em>LDAP search filter</em> option description contains more information on this type of authentication.</li></ul></blockquote>
//!  type: TYPE_STRING_LIST
//!  name: User auth: DN type
//
//! defvar: user_filter
//! This option is used when the <code>other</code> authentication scheme was selected as the value of the <em>DN type</em> option. The string here must be a valid LDAP filter string and you can use the following macros in it:<br /><blockquote><ul><li><code>%m</code> - mail address as typed by the user in the login screen. If the user typed only the user name part, default domain will be appended (if enabled). Note that in case the default domain is used and the default domains list contains more than one entry, the first entry will be used as a value of this macro.</li><li><code>%u</code> - user name as typed by the user.</li><li><code>%d</code> - domain name from the user's mail address (or from the default domain, if enabled). Note that in case the default domain is used and the default domains list contains more than one entry, the first entry will be used as a value of this macro.</li><li><code>%b</code> - base DN as specified in the LDAP options.</li><li><code>%p</code> - DN prefix as specified in the LDAP options.</li><ul></blockquote>
//!  type: TYPE_STRING
//!  name: User auth: LDAP search filter
//
//! defvar: user_uidfallback
//! If enabled, the failed <code>email</code> authentication will be attempted using the <code>uid</code> attribute as explained in the <em>DN type</em> description.
//!  type: TYPE_FLAG
//!  name: User auth: Fallback to UID
//
//! defvar: user_usedefdomain
//! If set then the default domain(s) will be used during authentication as explained in the description of the <em>DN type</em> option.
//!  type: TYPE_FLAG
//!  name: User auth: Use default domain
//
//! defvar: user_defdomains
//! This is a list of default domains, one per line. If more than one domain is listed here, then the login screen will contain a selection list with those domains in the order given below. If there's just one line in this box, it will be used silently when/if needed.
//!  type: TYPE_TEXT_FIELD
//!  name: User auth: Default domain(s)
//
//! defvar: ldap_basedn
//! Base DN (Distinguished Name) to be used in all the operations throughout the session in which this module was used to authenticate the users. 
//!  type: TYPE_STRING
//!  name: LDAP: base DN
//
//! defvar: ldap_dnprefix
//! String to be prepended to the base dn for every user DN generated in this module.
//!  type: TYPE_STRING
//!  name: LDAP: DN prefix
//
//! defvar: ldap_mailattr
//! Attribute to be used when matching user mail on searches. <font color='red'><strong>*** MUST NOT BE EMPTY ***</strong></font>
//!  type: TYPE_STRING
//!  name: LDAP: mail attribute
//
//! defvar: ldap_god_dn
//! The omnipotent DN that can do anything to the directory - used to retrieve user listings etc.
//!  type: TYPE_STRING
//!  name: LDAP: admin bind dn
//
//! defvar: ldap_god_pass
//! Password for the admin DN
//!  type: TYPE_STRING
//!  name: LDAP: admin bind password
//
//! defvar: ldap_protocol
//! The LDAP protocol version to use with this server
//!  type: TYPE_INT_LIST
//!  name: LDAP: protocol version to use
//
//! defvar: ldap_scope
//! Scope used by LDAP search operation.
//!  type: TYPE_STRING_LIST
//!  name: LDAP: query scope
//
